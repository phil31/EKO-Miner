...
static bool fan_sense_pass(GlobalState * GLOBAL_STATE)
{
    ...
    switch (GLOBAL_STATE->device_model) {
        ...
        case DEVICE_SUPRA:
            ...
            break;
        case DEVICE_EKOMB001:
            fan_speed = MAX6615_get_fan_speed();
            break;                    
        default:
    }
...
static bool power_consumption_pass(GlobalState * GLOBAL_STATE)
{
    float th_power_h = 0, th_power_l = 0;
    float power = 0;
    switch (GLOBAL_STATE->device_model) {
        case DEVICE_MAX:
        case DEVICE_ULTRA:
        case DEVICE_SUPRA:
            th_power_h = 15.0;
            th_power_l = 9.0;
            power = INA260_read_power() / 1000;
            break;
        case DEVICE_EKOMB001:
            th_power_h = 17.0;
            th_power_l = 14.0;
            power = INA745_read_power() / 1000;
            break;                    
        default:
    }

    ESP_LOGI(TAG, "Power: %f", power);
    if (power > th_power_l && power < th_power_h) {
        return true;
    }
    return false;
}
...
void self_test(void * pvParameters)
{
    ...
    switch (GLOBAL_STATE->device_model) {
        ...
        case DEVICE_ULTRA:
        case DEVICE_SUPRA:
            if (INA260_installed() && !power_consumption_pass(GLOBAL_STATE)) {
            ...
            break;
        case DEVICE_EKOMB001:
            if (INA745_installed() && !power_consumption_pass(GLOBAL_STATE)) {
                ESP_LOGE(TAG, "INA745 test failed!");
                display_msg("MONITOR:   FAIL", GLOBAL_STATE);
                return;
            }
            break;            
